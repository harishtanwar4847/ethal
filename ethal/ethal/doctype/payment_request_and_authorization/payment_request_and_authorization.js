...
frappe.ui.form.on('Payment Request and Authorization', {
	...
	amount: function(frm){
		...
		frappe.call({
			method: 'ethal.ethal.doctype.payment_request_and_authorization.payment_request_and_authorization.set_amount_in_words',
			args: {
				doc : frm.doc
			}
		})
		.success(success => {
			frm.set_value('amount_in_words', success.message)
		})
