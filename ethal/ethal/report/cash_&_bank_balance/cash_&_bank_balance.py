...
from erpnext.accounts.report.financial_statements \
	import filter_accounts, filter_out_zero_value_rows, get_additional_conditions
...
def set_gl_entries_by_account(
	...
	if accounts:
		...
		gl_entries = frappe.db.sql("""select `tabGL Entry`.posting_date, `tabGL Entry`.account, `tabGL Entry`.debit, `tabGL Entry`.credit, `tabGL Entry`.is_opening, `tabGL Entry`.fiscal_year,
			`tabGL Entry`.debit_in_account_currency, `tabGL Entry`.credit_in_account_currency, `tabGL Entry`.account_currency from `tabGL Entry`
			join `tabAccount` on `tabGL Entry`.account = `tabAccount`.name 
			where `tabAccount`.account_type in ('Cash', 'Bank') and 
			`tabGL Entry`.company=%(company)s
			{additional_conditions}
			and `tabGL Entry`.posting_date <= %(to_date)s
			order by `tabGL Entry`.account, `tabGL Entry`.posting_date""".format(additional_conditions=additional_conditions), gl_filters, as_dict=True) #nosec
